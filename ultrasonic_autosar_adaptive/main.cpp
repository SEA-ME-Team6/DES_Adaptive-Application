...
int32_t main() {
  ...
  if (bAraInitialized) {
    ara::log::Logger &araLog{ara::log::CreateLogger(
        ara::core::StringView{"ultr"},
        ara::core::StringView{"Logger for ultrasonic's main function."},
    ...
    ultrasonic ultrasonic_Obj;
    if (bProceed) {
      /* Initialize Functions */
      try {
        ultrasonic_Obj.initialize();
      } catch (std::exception const &e) {
    ...
    if (bProceed) {
      ...
      fcnExecutor.run(
          [&ultrasonic_Obj]() {
            return rtmGetStopRequested(ultrasonic_Obj.getRTM());
          },
          araLog);
#else
