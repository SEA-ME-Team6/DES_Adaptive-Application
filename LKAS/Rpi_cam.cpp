...
void Rpi_cam::step()
{
  static const std::array<int16_t, 8> tmp{ { 145, 500, 27, 639, 300, 300, 415,
      415 } };
  ...
  rtDW.s1DivS2 = rtDW.x_location * rtDW.win_y_high;
  rtDW.tformCompact_m[6] = rtDW.tformCompact[6] * rtDW.x_location;
  rtDW.tformCompact_m[7] = rtDW.tformCompact[7] * rtDW.x_location;
  ...
  for (rtDW.svdRsltVar = 0; rtDW.svdRsltVar < 20; rtDW.svdRsltVar++) {
    if (rtDW.inlierNum == 1) {
      ...
      if ((rtDW.win_y_low > 300.0) && (rtDW.win_y_low < 340.0)) {
        rtDW.x_location = rtDW.x_current - 220.0;
      }
    } else {
      rtDW.win_y_low = rtDW.bestInlierDis - ((static_cast<double>
        (rtDW.svdRsltVar) + 1.0) + 1.0) * 24.0;
      ...
      if ((rtDW.win_y_low > 300.0) && (rtDW.win_y_low < 340.0)) {
        rtDW.x_location = rtDW.x_current + 290.0;
      }
    }

    rtDW.win_y_low = 340.0 - rtDW.x_location;
  }

  // Send event
  ProvidedPort->Error.Send(rtDW.win_y_low);
}
