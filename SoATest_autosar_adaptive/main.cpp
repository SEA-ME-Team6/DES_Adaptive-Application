/* Code generated for Simulink model SoATest */
...
int32_t main() {
  ...
  if (bAraInitialized) {
    ara::log::Logger &araLog{ara::log::CreateLogger(
        ara::core::StringView{"SoAT"},
        ara::core::StringView{"Logger for SoATest's main function."},
    ...
    if (bProceed) {
      ...
      fcnExecutor.run(
          [&SoATest_Obj]() {
            return rtmGetStopRequested(SoATest_Obj.getRTM());
          },
          araLog);
#else
